...
(define (eval-under-env e env)
  (cond [(mul461-var? e) 
        ...
        [(mul461-int? e)
         (mul461-int (mul461-int-value e))]
        [(mul461-bool? e)
         (mul461-bool (mul461-bool-value e))]
        [(mul461-null? e)
         (mul461-null)]
        [(mul461-cons? e)
        ...
        [(mul461-let? e)
         (let ([v1 (eval-under-env (mul461-let-e e) env)]
               [v2 (mul461-let-name e)])
           (eval-under-env (mul461-let-body e) (cons (cons v2 v1) env)))]
        [(mul461-fun? e)
         (mul461-closure e env)]
        [(mul461-+? e) 
        ...
        [(mul461-cdr? e)
         (let ([v (eval-under-env (mul461-cdr-e e) env)])
           (if (mul461-cons? v)
               (mul461-cons-e2 v)
               (error "mul461-cdr applied to non-pair")))]
        [(mul461-isnull? e)
         (let ([v (eval-under-env (mul461-isnull-e e) env)])
           (if (mul461-null? v)
               (mul461-bool #t)
               (mul461-bool #f)))]
        [(mul461-call? e)
         (let ([v1 (eval-under-env (mul461-call-funexp e) env)]
           ...
           (if (mul461-closure? v1)
               (let ([v3 (mul461-closure-fun v1)]
                     [v4 (mul461-closure-env v1)])
                 (if (mul461-fun? v3)
                     (eval-under-env
                      (mul461-fun-body v3)
                      (cons (cons (mul461-fun-formal v3) v2) (cons (cons (mul461-fun-fname v3) v1) v4)))
                     (error "mul461-closure applied to non-function in mul461-call")))
...
(define (makelet* t e)
  (if (null? t)
      e
      (foldr (lambda (p e) (mul461-let (car p) (cdr p) e)) e t)))
...
(define (makelist t)
  (if (null? t)
      (mul461-null)
      (foldr mul461-cons (mul461-null) t)))
...
(define mul461-filter
  (mul461-fun
   ...
   (mul461-fun
    "filterf" "t"
    (mul461-if (mul461-isnull (mul461-var "t"))
     (mul461-null)
     (mul461-if (mul461-call (mul461-var "f") (mul461-car (mul461-var "t")))
                (mul461-cons (mul461-car (mul461-var "t"))
